...
def _init():
    """Initialise embedding model once pet process. Called lazily on first use."""
...
def _embed_batch(batch: list[str]) -> list[list[float]]:
    if _model_type == "gemini":
        # Exponential backoff: 1 s -> 2 s -> 4s -> 8 s (4 attempts total)
...
def embed_texts(texts: list[str]) -> list[list[float]]:
    ...
    for i in range(0, len(texts), BATCH_SIZE):
        ...
        with logfire.span("Embed Batch", model=_active_model, start=i, size=len(batch)):
